...
@since   10/02/2017
@version P12
@obs 	 Criado inicialmente para enviar aviso a contabilidade quando o contrato iniciar vigencia [Situacao=05]
...
user function CN100SIT()
	...
	Local _cAvCdRev	  := GetNewPar("AB_AVSTCTO","05/06")						 	// Codigos das reviões que enviaram aviso a contabilidade
	Local cEMAILREM   := GetNewPar("AB_MAILREM","workflow@apex.brasil.com.br" )		// E-mail do remetente (APEX)
	...
	If _cSituac $ _cAvCdRev
		...
		cCORPO += "<td><center>" +CN9->CN9_NUMERO+" R"+CN9->CN9_REVISA + "</center></td>" + CRLF
		...
		MsgInfo("Aviso de e-mail de mudança de situação enviado à contabilidade.")
	Endif
